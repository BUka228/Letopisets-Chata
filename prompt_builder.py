...
def build_reply_to_intervention_prompt(
    ...
    prompt = (
        ...
        f"6. не используй торчки в конце совего высказывания. Другие знаки припинания можешь использовать. Если точка используется между твоими предложениями, то ее можно использоать, главное чтобы ее не было в самом конце.\n"

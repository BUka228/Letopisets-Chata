...
def build_summary_content(messages: List[Dict[str, Any]]) -> Optional[PreparedContent]:
    ...
    initial_prompt = (
        ...
        "Твоя задача — создать **краткую выжимку (summary)**, отражающую **основные темы обсуждений, ключевые вопросы, принятые решения или важные события**, произошедшие за этот период. "
        "Упоминай основных участников обсуждений по их *именам*. Используй **Markdown** для списков или коротких абзацев (не более 2-3). "
        "Игнорируй приветствия, прощания, короткие односложные ответы и малозначительные сообщения (флуд). "
        "Сосредоточься на сути дискуссий. Выжимка должна быть лаконичной и информативной.\n\n"
